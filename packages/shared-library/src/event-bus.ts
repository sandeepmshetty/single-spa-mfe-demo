...
export class EventBus {
   ...
   */
  onAll(callback: (payload: EventPayload) => void): () => void {
   ...
   */
  onFromSource(source: string, callback: (payload: EventPayload) => void): () => void {
    const subscription = this.eventSubject.subscribe((payload) => {
  ...
  private getCurrentMFE(): string {
    ...
    if (hostname.includes('react')) return MFE_NAMES.REACT;
    if (hostname.includes('vue')) return MFE_NAMES.VUE;
    if (hostname.includes('angular')) return MFE_NAMES.ANGULAR;
    if (hostname.includes('shared')) return MFE_NAMES.SHARED;
